...
export function useAddUserAccount() {
    ...
    return useToastMutation(
        async (data: { apiKey: string; type: AccountType; apiUrl?: string }) => {
            let user;

            // nzbdav doesn't support CORS, skip browser validation
            if (data.type === AccountType.NZBDAV) {
                user = {
                    id: `nzbdav-${data.apiKey.substring(0, 8)}`,
                    type: data.type,
                    apiKey: data.apiKey,
                    apiUrl: data.apiUrl,
                    name: "nzbdav",
                ...
                const ClientClass = getClient({ type: data.type });
                user = await ClientClass.getUser(data.apiKey);
            }
...
export async function getDebridUserInfo(account: UserAccount) {
    // nzbdav doesn't support CORS, return mock user info
    // The actual API calls happen server-side
    if (account.type === AccountType.NZBDAV) {
        return {
            id: account.id,
            type: AccountType.NZBDAV,
            apiKey: account.apiKey,
            apiUrl: account.apiUrl ?? undefined,
            name: account.name,
            email: "nzbdav@local",
            language: "en",
            isPremium: true,
    ...
    return await ClientClass.getUser(account.apiKey);
}

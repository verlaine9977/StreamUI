...
export function AddAccountForm() {
    ...
    const isNzbdav = selectedType === AccountType.NZBDAV;

    // Set default API URL when nzbdav is selected
    useEffect(() => {
        if (isNzbdav) {
            const currentUrl = form.getValues("apiUrl");
            if (!currentUrl) {
                form.setValue("apiUrl", "http://nzbdav:3000");
            }
        }
    }, [isNzbdav, form]);
                    ...
                    <div className="flex flex-col gap-5">
                        ...
                        {isNzbdav && (
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel className="text-xs text-muted-foreground">API URL</FormLabel>
                                        <FormControl>
                                            <Input {...field} placeholder="http://your-nzbdav-server:port" />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}

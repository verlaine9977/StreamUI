...
export const PlayUrlButton = memo(
    function PlayUrlButton({ url, title, onOpenPreview, onPlay }: PlayUrlButtonProps) {
        const handlePlay = () => {
            onPlay?.();
            if (onOpenPreview) {
    ...
    (prev, next) => prev.url === next.url && prev.title === next.title && prev.onOpenPreview === next.onOpenPreview && prev.onPlay === next.onPlay
);

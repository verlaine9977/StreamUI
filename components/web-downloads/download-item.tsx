...
import { formatSize, cn, getFileType, playUrl, copyPlayerUrl } from "@/lib/utils";
...
import { Copy, Trash2, Loader2, Download, PlayCircle, View, Link2 } from "lucide-react";
...
export const DownloadItem = memo(function DownloadItem({
    ...
    const [loading, setLoading] = useState<"copy" | "download" | "preview" | "copy-player-url" | null>(null);
    ...
    const getLink = async (action: "copy" | "download" | "preview" | "copy-player-url") => {
        setLoading(action);
        try {
    ...
    const handleCopyPlayerUrl = async () => {
        const link = await getLink("copy-player-url");
        if (link) {
            await copyPlayerUrl({ url: link, fileName: download.name, player: mediaPlayer });
        }
    };
                            ...
                            <>
                                ...
                                {usesExternalPlayer && (
                                    <Button
                                        ...
                                        {loading === "copy-player-url" ? (
                                            <Loader2 className="size-3.5 md:size-4 animate-spin" />
                                        ) : (
                                            <Link2 className="size-3.5 md:size-4" />
                                        )}
                                    </Button>
                                )}
                                <Button

...
import { ChevronRight, Copy, Download, CirclePlay, Loader2, Link2 } from "lucide-react";
import { cn, getFileType } from "@/lib/utils";
import { formatSize, playUrl, downloadLinks, copyLinksToClipboard, copyPlayerUrl } from "@/lib/utils";
...
const FileActionButton = memo(function FileActionButton({
    ...
    node: DebridFileNode;
    action: "copy" | "download" | "play" | "copy-player-url";
}) {
    ...
    const handleAction = async (linkInfo: DebridLinkInfo) => {
        ...
        switch (action) {
            ...
            case "copy-player-url":
                await copyPlayerUrl({ url: linkInfo.link, fileName: linkInfo.name });
                break;
        }
    };
    ...
    const icon = useMemo(() => {
        ...
        return {
            ...
            "copy-player-url": <Link2 className="h-3 w-3 sm:h-3.5 sm:w-3.5" />,
        }[action];
                    ...
                    <div className="flex gap-2 md:gap-0.5">
                        ...
                        {fileType === FileType.VIDEO && !isVideoWithBrowserPlayer && (
                            <>
                                ...
                                <FileActionButton node={node} action="copy-player-url" />
                            </>
                        )}

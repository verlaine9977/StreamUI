...
export async function getUserAccounts() {
    ...
    const accounts = await db.select().from(userAccounts).where(eq(userAccounts.userId, SINGLE_USER.id));
 ...
 */
export async function addUserAccount(data: { apiKey: string; type: AccountType; name: string; apiUrl?: string }) {
    const session = await getSession();
    ...
    const existing = await db
        ...
        .where(
            and(
                eq(userAccounts.userId, SINGLE_USER.id),
...
export async function removeUserAccount(accountId: string) {
    ...
    const account = await db
        .select()
        .from(userAccounts)
        .where(and(eq(userAccounts.id, accountId), eq(userAccounts.userId, SINGLE_USER.id)));

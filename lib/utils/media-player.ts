...
const generateVlcUrl = (url: string, fileName: string): string => {
    ...
    // macOS - use x-callback-url format which is more reliable
    if (platform === Platform.MACOS) {
        return `vlc-x-callback://x-callback-url/stream?url=${encodeURIComponent(url)}`;
    }

    // Windows/Linux - standard vlc:// protocol
    return `vlc://${url}`;
};
...
/**
 * Generate external player URL without opening it
 */
export const getPlayerUrl = ({
    ...
    return PLAYER_URLS[selectedPlayer](url, fileName);
};

/**
 * Copy external player URL to clipboard
 */
export const copyPlayerUrl = async ({
    url,
    fileName,
    player,
}: {
    ...
    const playerUrl = getPlayerUrl({ url, fileName, player });

    if (!playerUrl) {
        toast.error("Browser player does not support external URLs. Please select an external player in settings.");
        return false;
    }
